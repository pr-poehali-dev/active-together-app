...
type Role = "student" | "teacher" | "parent" | "mentor";

const roles: { id: Role; icon: string; label: string; desc: string }[] = [
  { id: "student", icon: "🎒", label: "Ученик", desc: "Участник проекта" },
  { id: "teacher", icon: "👨‍🏫", label: "Учитель", desc: "Управление классами" },
  { id: "parent", icon: "👨‍👩‍👧", label: "Родитель", desc: "Следить за ребёнком" },
  { id: "mentor", icon: "🌟", label: "Наставник", desc: "8–9 классы" },
];
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="phone-frame bg-background font-golos flex flex-col min-h-dvh">
      {/* Status bar */}
      ...
      <div className="flex-1 flex flex-col px-5 pt-6 pb-8 overflow-y-auto">
        ...
        <div className="text-center mb-8 animate-slide-up">
          <div
            className="w-20 h-20 rounded-3xl mx-auto mb-4 flex items-center justify-center text-4xl animate-pulse-green"
            style={{ background: "linear-gradient(135deg, var(--green), #00d4aa)" }}
          >
            🏃
          </div>
          <h1 className="font-oswald text-3xl font-bold tracking-wide text-gradient-green">
            АКТИВ.ВМЕСТЕ
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Единый штаб здоровья и активности
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {roles.map((role) => (
              <button
                ...
                className="p-3 rounded-2xl text-left transition-all duration-200 active:scale-95"
                style={{
                  background:
                    selectedRole === role.id
                      ? "linear-gradient(135deg, hsl(142 72% 50% / 0.18), hsl(142 72% 50% / 0.06))"
                      : "hsl(var(--card))",
                  border:
                    selectedRole === role.id
                      ? "1.5px solid hsl(142 72% 50% / 0.5)"
              ...
              >
                ...
                <p
                  ...
                  style={{
                    color: selectedRole === role.id ? "var(--green)" : "hsl(var(--foreground))",
                  }}
                >
                  {role.label}
                </p>
        ...
        {/* Form */}
        <div className="space-y-3 mb-5 animate-slide-up delay-200">
          {/* Login */}
          <div>
            ...
            <div
              ...
              style={{
                ...
                border: `1.5px solid ${login ? "hsl(142 72% 50% / 0.4)" : "hsl(var(--border))"}`,
                transition: "border-color 0.2s",
              }}
            >
              <Icon name="User" size={18} className="text-muted-foreground flex-shrink-0" />
              <input
                type="text"
          ...
          <div>
            ...
            <div
              ...
              style={{
                background: "hsl(var(--card))",
                border: `1.5px solid ${password ? "hsl(142 72% 50% / 0.4)" : "hsl(var(--border))"}`,
                transition: "border-color 0.2s",
              }}
            >
              <Icon name="Lock" size={18} className="text-muted-foreground flex-shrink-0" />
              <input
                ...
                className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground"
              />
              <button onClick={() => setShowPass((p) => !p)} className="flex-shrink-0">
                <Icon
          ...
          {/* Error */}
          {error && (
            <p className="text-xs font-semibold text-center" style={{ color: "hsl(var(--destructive))" }}>
              {error}
            </p>
          )}
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-4 rounded-2xl font-bold text-sm transition-all active:scale-95 animate-slide-up delay-300 flex items-center justify-center gap-2"
          style={{
            background: loading
              ? "hsl(var(--muted))"
              : "linear-gradient(135deg, var(--green), #00d4aa)",
        ...
        {/* Forgot password */}
        <button className="mt-4 text-xs text-muted-foreground text-center w-full animate-slide-up delay-400">
          Забыл пароль? Обратись к учителю
        </button>
      </div>
    </div>
  );
